...
inline void
Log( ESysLogMessageType _eslmtType, std::string _ss, ... )
{
  // We add <type>: to the start of the format string.
  std::string strFmtAnnotated;
...
inline void
LogException( exception const & _rexc, const char * _pcFile, unsigned int _nLine, const char * _pcFmt, ... )
{
  // We add [type]:_psFile:_nLine: to the start of the format string.
  std::string strFmtAnnotated;
  { // B
    // Add the exception description at the end of the error string.
    PrintfStdStr( strFmtAnnotated, "Exception:%s:%d: %s, what()[%s]", _pcFile, _nLine, _pcFmt, _rexc.what() );
  }
...
void
_SysLogThreadHeader::ToJSONStream( JsonValueLife< t_tyJsonOutputStream > & _jvl ) const
{
...
void
_SysLogThreadHeader::FromJSONStream( JsonReadCursor< t_tyJsonInputStream > & _jrc )
{
  Clear();
  // We should be in an object in the JSONStream and we will add our (key,values) to this object.
    ...
    if ( _jrc.FMoveDown() )
    {
      for ( ; !_jrc.FAtEndOfAggregate(); (void)_jrc.FNextElement() )
      {
        ...
        if ( fGetKey )
        {
          ...
          else if ( ( ejvtTrue == jvtValue ) || ( ejvtFalse == jvtValue ) )
          {
...
template < class t_tyJsonOutputStream >
void
_SysLogContext::ToJSONStream( JsonValueLife< t_tyJsonOutputStream > & _jvl ) const
{
  // We should be in an object in the JSONStream and we will add our (key,values) to this object.
  Assert( _jvl.FAtObjectValue() );
  if ( _jvl.FAtObjectValue() )
...
template < class t_tyJsonInputStream >
void
_SysLogContext::FromJSONStream( JsonReadCursor< t_tyJsonInputStream > & _jrc )
{
  Clear();
  // We should be in an object in the JSONStream and we will add our (key,values) to this object.
  Assert( _jrc.FAtObjectValue() );
...
template < const int t_kiInstance > _SysLogMgr< t_kiInstance >::~_SysLogMgr() { CloseSysLogFile(); }
                                                          ...
                                                          bool _fIsMainThread, const char * _pszLogDir )
{
  ...
  if ( !!pszProgNameNoPath )
  {
    ...
    pszProgNameNoPath = _pszProgramName;
  ...
  slth.m_szProgramName +=
      pszProgNameNoPath ? pszProgNameNoPath : "ERRORPROGRAMNAME"; // Put full path to EXE here for disambiguation when working with multiple versions.
  ...
  if ( !_pszLogDir ) // legacy operation - we are putting the logfile in the same directory as the "executable" where the
  // caller may fix up that path to be the log directory and the executable name is there just to find it - it isn't in the log dir...
  {
  ...
  { // more flexible operation - we are putting the logfile in a directory specified by the caller.
    strLogFile = _pszLogDir;
    if ( strLogFile[ strLogFile.length() - 1 ] != TChGetFileSeparator< char >() )
      strLogFile += TChGetFileSeparator< char >();
    strLogFile += pszProgNameNoPath ? pszProgNameNoPath : "ERRORPROGRAMNAME";
  }
...
template < const int t_kiInstance >
void
_SysLogMgr< t_kiInstance >::CloseSysLogFile() noexcept( true )
{
...
// This failure mimicks the ANSI standard failure: Print a message (in our case to the syslog and potentially as well to the screen) and then flush the log file
// and abort() (if _fAbort).
inline void
                         ...
                         unsigned int _nLine, const char * _szFunction, const char * _szMesg, ... )
{
  if ( !n_SysLog::FSetInAssertOrVerify( true ) )
  {
    ...
    try
    {
      ...
      // We must check if there are any % signs in _szAssertion and substitute %% for each - otherwise these can cause the program to ABORT - which we do not
      // prefer.
      ...
      // We log both the full string - which is the only thing that will end up in the syslog - and each field individually in JSON to allow searching for
      // specific criteria easily.
      (void)FPrintfStdStrNoThrow( strFmt, !strMesg.length() ? "%s:[%s:%d],%s(): %s." : "%s:[%s:%d],%s: %s. %s", _szAssertVerify, _szFile, _nLine, _szFunction,
                                  strAssertion.c_str(), strMesg.c_str() );
      ...
      else if ( eabiThrowException == _eabi )
      { // Take care of this to avoid the catch and rethrow.
      }
      else if ( eabiBreak == _eabi )
      {
        (void)n_SysLog::FSetInAssertOrVerify(
            false ); // Change this here since we may be able to change lines in a method above us in the debugger in VC for instance.
        DEBUG_BREAK;
      }
    }
    catch ( ... )
...
inline void
Trace_LogMessageVArg( EAbortBreakIgnore _eabi, const char * _szFile, unsigned int _nLine, const char * _szFunction,
                      const n_SysLog::vtyJsoValueSysLog * _pjvTrace, const char * _szMesg, va_list _ap )
{
  try
  {
    std::string strMesg;
...
inline void
Trace_LogMessage( EAbortBreakIgnore _eabi, const char * _szFile, unsigned int _nLine, const char * _szFunction, const n_SysLog::vtyJsoValueSysLog * _pjvTrace,
                  const char * _szMesg, ... )
{
  va_list ap;
  if ( !!_szMesg && !!*_szMesg )
