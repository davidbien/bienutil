...
// MapToIntType
template < size_t t_stMaxSize > struct MapToIntType
{
  typedef uint8_t _tyType;
};
template <> struct MapToIntType< USHRT_MAX >
{
  typedef uint16_t _tyType;
};
template <> struct MapToIntType< UINT_MAX >
{
  typedef uint32_t _tyType;
};
template <> struct MapToIntType< ULLONG_MAX >
{
  typedef uint64_t _tyType;
};
template < size_t t_stMaxSize > using t_tyMapToIntType = typename MapToIntType< t_stMaxSize >::_tyType;
...
template < class t_tyUint >
constexpr t_tyUint
UpperSqrt( const t_tyUint _ui )
{
  if ( !FIsPow2( _ui ) )
    throw std::logic_error( "UpperSqrt(): _ui is not a power of two." );
  // Note that this will return 1 as the sqrt of 2. Probably we would return 2 but it doesn't matter for the Van Emde Boas tree impl because we should never
  // call this with 2.
...
// Count leading zeros
template < typename t_TyInt >
t_TyInt
Clz( t_TyInt _n );

template <>
inline uint32_t
Clz< uint32_t >( uint32_t _n )
{
#if defined( _MSC_VER )
  unsigned long index;
  if ( _BitScanReverse( &index, _n ) )
    return 31 - index;
  return 32;
#elif defined( __GNUC__ ) || defined( __clang__ )
  return _n ? __builtin_clz( _n ) : 32;
#else
...
template <>
inline uint64_t
Clz< uint64_t >( uint64_t _n )
{
#if defined( _MSC_VER )
  unsigned long index;
#if defined( _WIN64 )
  if ( _BitScanReverse64( &index, _n ) )
    return 63 - index;
#else
  // Handle 64-bit integers on 32-bit Windows
  unsigned long high;
  if ( _BitScanReverse( &high, (uint32_t)( _n >> 32 ) ) )
    return 31 - high;
  if ( _BitScanReverse( &index, (uint32_t)_n ) )
    return 63 - index;
#endif
  return 64;
#elif defined( __GNUC__ ) || defined( __clang__ )
  return _n ? __builtin_clzll( _n ) : 64;
#else
  // Fallback implementation
  if ( !_n )
    return 64;
...
template <>
inline uint32_t
Ctz< uint32_t >( uint32_t _n )
{
#if defined( _MSC_VER )
  unsigned long index;
  if ( _BitScanForward( &index, _n ) )
    return index;
  return 32;
#elif defined( __GNUC__ ) || defined( __clang__ )
  return _n ? __builtin_ctz( _n ) : 32;
#else
  // Fallback implementation
  if ( !_n )
    return 32;
...
template <>
inline uint64_t
Ctz< uint64_t >( uint64_t _n )
{
#if defined( _MSC_VER )
  unsigned long index;
#if defined( _WIN64 )
  if ( _BitScanForward64( &index, _n ) )
    return index;
#else
  // Handle 64-bit integers on 32-bit Windows
  if ( _BitScanForward( &index, (uint32_t)_n ) )
    return index;
  if ( _BitScanForward( &index, (uint32_t)( _n >> 32 ) ) )
    return index + 32;
#endif
  return 64;
#elif defined( __GNUC__ ) || defined( __clang__ )
  return _n ? __builtin_ctzll( _n ) : 64;
#else
  // Fallback implementation
  if ( !_n )
    return 64;
...
class _VebFixedBase
{
  ...
  template < size_t, class, class > friend class VebTreeWrap;
...
public:
  ...
  bool FEmpty( bool = false ) const { return !FHasAnyElements(); }
  ...
  void Clear() { memset( m_rgUint, 0, sizeof( m_rgUint ) ); }
...
template < size_t t_kstUniverse, bool t_kfContainedInVebWrap = false > class VebTreeFixed;

// Specialize <2> separately since we can probably do a bit better than above (though certainly not much).
template < bool t_kfContainedInVebWrap > class VebTreeFixed< 2, t_kfContainedInVebWrap >
{
...
public:
  ...
  void swap( _tyThis & _r ) { std::swap( m_byVebTree2, _r.m_byVebTree2 ); }
  ...
  bool FEmpty( bool = false ) const { return !FHasAnyElements(); }
  bool FHasAnyElements() const { return m_byVebTree2 != 0b00; }
  bool FHasOneElement() const { return ( m_byVebTree2 == 0b01 ) || ( m_byVebTree2 == 0b10 ); }
  bool FHasMinMax( _tyImplType * _pnMin, _tyImplType * _pnMax )
  {
    if ( FHasAnyElements() )
    {
  ...
  bool FHasElement( _tyImplType _x ) const { return !!( m_byVebTree2 & ( _tyImplType( 1 ) << _x ) ); }
  // Return the next element after _x or 0 if there is no such element.
  _tyImplType NSuccessor( _tyImplType _x ) const
  {
    Assert( _x < s_kstUniverse );
...
protected:
  void _ClearHasElements() { m_byVebTree2 = 0b00; }
...
template < bool t_kfContainedInVebWrap > class VebTreeFixed< 4, t_kfContainedInVebWrap > : public _VebFixedBase< t_kfContainedInVebWrap, uint8_t, 1, 4 >
{
...
template < bool t_kfContainedInVebWrap > class VebTreeFixed< 8, t_kfContainedInVebWrap > : public _VebFixedBase< t_kfContainedInVebWrap, uint8_t, 1 >
{
...
template < bool t_kfContainedInVebWrap > class VebTreeFixed< 16, t_kfContainedInVebWrap > : public _VebFixedBase< t_kfContainedInVebWrap, uint16_t, 1 >
{
...
template < bool t_kfContainedInVebWrap > class VebTreeFixed< 32, t_kfContainedInVebWrap > : public _VebFixedBase< t_kfContainedInVebWrap, uint32_t, 1 >
{
...
template < bool t_kfContainedInVebWrap > class VebTreeFixed< 64, t_kfContainedInVebWrap > : public _VebFixedBase< t_kfContainedInVebWrap, uint64_t, 1 >
{
...
template < bool t_kfContainedInVebWrap > class VebTreeFixed< 128, t_kfContainedInVebWrap > : public _VebFixedBase< t_kfContainedInVebWrap, uint64_t, 2 >
{
...
template < bool t_kfContainedInVebWrap > class VebTreeFixed< 256, t_kfContainedInVebWrap > : public _VebFixedBase< t_kfContainedInVebWrap, uint64_t, 4 >
{
...
template < bool t_kfContainedInVebWrap > class VebTreeFixed< 512, t_kfContainedInVebWrap > : public _VebFixedBase< t_kfContainedInVebWrap, uint64_t, 8 >
{
...
template < size_t t_kstUniverse, bool t_kfContainedInVebWrap > class VebTreeFixed
{
  typedef VebTreeFixed _tyThis;
...
#pragma GCC diagnostic pop
  ...
  static size_t STClusters() { return s_kstUniverseSqrtLower; }
  ...
  static _tyImplType NIndex( _tyImplTypeSubtree _x, _tyImplTypeSubtree _y ) { return ( _x * s_kstUniverseSqrtUpper ) + _y; }
  ...
  bool FHasAnyElements() const { return _NMax() >= _NMin(); }
  bool FHasOneElement() const { return _NMin() == _NMax(); }
  bool FHasMinMax( _tyImplType * _pnMin, _tyImplType * _pnMax )
  {
    if ( FHasAnyElements() )
    {
    ...
    if ( fElsCluster && ( nEl < nMaxCluster ) )
    {
      ...
      if ( nMinCluster == nMaxCluster )
      {
        ...
        if ( NIndex( nCluster, nMaxCluster ) == m_nMax )
        {
          ...
          m_nMax = ( s_kstitNoPredecessorSummaryTree == nPredecessiveCluster ) ? _NMin()
                                                                               : NIndex( nPredecessiveCluster, m_rgstSubtrees[ nPredecessiveCluster ].NMax() );
        }
      }
      else if ( NIndex( nCluster, nOffsetSubtree ) == m_nMax )
        m_nMax = NIndex( nCluster, m_rgstSubtrees[ nCluster ].NMax() );
      return NIndex( nCluster, nOffsetSubtree );
    }
    else
  ...
  _tyThis & operator|=( _tyThis const & _r )
  {
    ...
    do
    {
      ...
      if ( !rstThis.FHasAnyElements() )    // REVIEW: Could integrate the transition from "no elements"->"has elements" into the |= call below to avoid multiple
                                           // passes on rstThis.
...
protected:
  ...
  _tyImplType _NMin() const { return m_nMinPlusOne - 1; }
  void _SetMin( _tyImplType _nMin ) { m_nMinPlusOne = _nMin + 1; }
  _tyImplType _NMax() const { return m_nMax; }
  void _SetMax( _tyImplType _nMax ) { m_nMax = _nMax; }
...
#pragma GCC diagnostic pop
  ...
  static size_t NUniverse() { return s_kstUniverse; }
  // Returns the size for this set of element - with which it was constucted or initialized.
  size_t NSize() const { return m_nLastElement + 1; }
  ...
  static _tyImplType NIndex( _tyImplTypeSubtree _x, _tyImplTypeSubtree _y ) { return ( _x * t_kstUniverseCluster ) + _y; }
  ...
  bool FHasAnyElements() const { return m_nMax >= m_nMin; }
  bool FHasOneElement() const { return m_nMin == m_nMax; }
  size_t STClusters() const { return m_rgstSubtrees.size(); }
  bool FHasMinMax( _tyImplType * _pnMin, _tyImplType * _pnMax )
  {
    if ( FHasAnyElements() )
    {
  ...
  void Resize( size_t _stNewUniverse, bool _fShrinkReserve )
  {
    ...
    if ( _stNewUniverse > NSize() )
    {
      ...
      if ( stClustersNew > m_rgstSubtrees.size() )
      {
        ...
        Assert( !_fShrinkReserve ||
                ( m_rgstSubtrees.size() == m_rgstSubtrees.capacity() ) ); // If this Assert fails that is ok, but we would prefer to have it this way.
      }
    ...
    if ( _fShrinkReserve )
    {
      ...
      m_rgstSubtrees.shrink_to_fit(); // might throw and if so we will be correct but will have allocated too much memory and the summary will think it has more
                                      // elements but they will be empty so that is ok.
  ...
  void InsertAll( const _tyImplType * _pnFirstInsert = nullptr, const _tyImplType * _pnLastElement = nullptr )
  {
    ...
    Assert( !_pnLastElement || *_pnLastElement == m_nLastElement ); // We should always be specifying everything - we don't use either of the arguments below
                                                                    // because there is no need to support them in this method at least.
    // Algorithm:
    // Set m_nMin to 0, set m_nMax to m_nLastElement.
    // For first cluster call InsertAll with (min,min) to skip inserting the minth element.
    m_nMin = 0;
  ...
  size_t NSuccessor( size_t _x = ( numeric_limits< size_t >::max )() ) const
  {
    if ( !FHasAnyElements() )
      return s_kitNoSuccessor;
    else if ( ( ( numeric_limits< size_t >::max )() == _x ) || ( _x < m_nMin ) )
      return m_nMin;
    ...
    if ( fElsCluster && ( nEl < nMaxCluster ) )
    {
      ...
      if ( nMinCluster == nMaxCluster )
      {
        ...
        if ( NIndex( nCluster, nMaxCluster ) == m_nMax )
        {
          ...
          //  removed m_nMax and m_nMin is the only element left.
          _tyImplTypeSummaryTree nPredecessiveCluster = m_stSummary.NPredecessor( nCluster );
          m_nMax = ( s_kstitNoPredecessorSummaryTree == nPredecessiveCluster ) ? m_nMin
                                                                               : NIndex( nPredecessiveCluster, m_rgstSubtrees[ nPredecessiveCluster ].NMax() );
        }
      }
      else if ( NIndex( nCluster, nOffsetSubtree ) == m_nMax )
  ...
  size_t NPredecessor( size_t _x = ( numeric_limits< size_t >::max )() ) const
  {
    if ( !FHasAnyElements() )
      return s_kitNoPredecessor;
    else if ( ( numeric_limits< size_t >::max )() == _x )
      return m_nMax;
    if ( _x > m_nLastElement )
      THROWNAMEDEXCEPTION( "_x[%zu] is greater than m_nLastElement[%zu].", size_t( _x ), size_t( m_nLastElement ) );
    if ( _x > m_nMax )
  ...
  size_t NPredecessorDelete( size_t _x = ( numeric_limits< size_t >::max )() )
  {
    if ( !FHasAnyElements() )
      return s_kitNoPredecessor;
    else if ( ( ( numeric_limits< size_t >::max )() == _x ) || ( ( _x > m_nMax ) && ( _x <= m_nLastElement ) ) )
    {
      _tyImplType n = m_nMax;
      Delete( m_nMax );
      return n;
  ...
  _tyThis & BitwiseInvert()
  {
    ...
    for ( _tyImplTypeSummaryTree nClusterCur = 0; pstEnd != pstCur; ++pstCur, ++nClusterCur )
    {
      ...
      pstCur->BitwiseInvert( ( pstEnd - 1 == pstCur ) ? pnLastElementSubtree
                                                      : 0 ); // Pass in any limit on the last cluster to avoid setting bits beyond the end.
      if ( !fInSummary )
        m_stSummary.Insert( nClusterCur );
      _tyImplTypeSubtree nMaxCluster;
...
protected:
  ...
  void _ShrinkMemory() { m_rgstSubtrees.shrink_to_fit(); }
