...
struct JsonCharTraits< char >
{
    ...
    static bool FIsWhitespace( _tyUtfChar _utc )
    {
        return ( 1 == _utc.NCodes() ) && ( ( ' ' == *_utc.begin() ) || ( '\t' == *_utc.begin() ) || ( '\n' == *_utc.begin() ) || ( '\r' == *_utc.begin() ) );
    }
...
struct JsonCharTraits< wchar_t >
{
    ...
    static bool FIsWhitespace( _tyUtfChar _tc )
    {
        return ( ( L' ' == _tc ) || ( L'\t' == _tc ) || ( L'\n' == _tc ) || ( L'\r' == _tc ) );
    }
    static size_t StrLen( _tyLPCSTR _psz )
...
public:
    ...
    typedef typename _tyCharTraits::_tyUtfChar _tyUtfChar;
    ...
    void SkipWhitespace() 
    {
        ...
        if ( m_fHasLookahead && !_tyCharTraits::FIsWhitespace( m_utcLookahead ) )
            return;
        ssize_t sstRead;
        for ( ; ; )
        {
            _tyChar tchFirst;
            ssize_t sstRead = read( m_fd, &tchFirst, sizeof tchFirst );
            if ( -1 == sstRead )
                THROWBADJSONSTREAMERRNO( errno, "JsonLinuxInputStream::SkipWhitespace(): read() failed for file [%s]", m_szFilename.c_str() );
            m_pos += sstRead;
            if ( sstRead != sizeof tchFirst )
            ...
            // We must read any multibyte UTF:
            unsigned int nCharsLeft = m_utcLookahead.NSetFirstChar( tchFirst );
            if ( nCharsLeft )
            {
                sstRead = read( m_fd, m_utcLookahead.begin()+1, nCharsLeft * sizeof(_tyChar) );
                if ( sstRead != nCharsLeft * sizeof(_tyChar) )
                {
                    if ( -1 == sstRead )
                        THROWBADJSONSTREAMERRNO( errno, "JsonLinuxInputStream::SkipWhitespace(): read() failed for file [%s] reading multicode UTF.", m_szFilename.c_str() );
                    else
                    {
                        m_pos += sstRead;
                        THROWBADJSONSTREAMERRNO( errno, "JsonLinuxInputStream::SkipWhitespace(): missing multicode bytes before EOF [%ld].", 
                            m_szFilename.c_str(), nCharsLeft * sizeof(_tyChar) - sstRead );
                    }
                }
                m_pos += sstRead;
            }

            if ( !_tyCharTraits::FIsWhitespace( m_utcLookahead ) )
            {
    ...
    _tyFilePos PosGet() const
    {
        ...
        if ( m_fUseSeek )
        {
            ...
            if ( m_fHasLookahead )
                pos -= m_utcLookahead.NCodes() * sizeof(_tyChar); // Since we have a lookahead we are actually one UTF character before.
            return pos;
        }
        else
            return m_pos - ( m_fHasLookahead ? ( m_utcLookahead.NCodes() * sizeof(_tyChar) ) : 0 );
    }
    // Read a single character from the file - always throw on EOF.
    _tyUtfChar ReadChar( const char * _pcEOFMessage )
    {
        assert( FOpened() );
        if ( m_fHasLookahead )
        {
            m_fHasLookahead = false;
            return m_utcLookahead;
        }
        _tyChar tchFirst;
        ssize_t sstRead = read( m_fd, &tchFirst, sizeof tchFirst );
        if ( sstRead != sizeof tchFirst )
        ...
        if ( nCharsLeft )
        {
            ...
            {
                if ( -1 == sstRead )
                    THROWBADJSONSTREAMERRNO( errno, "JsonLinuxInputStream::ReadChar(): read() failed for file [%s] reading multicode UTF.", m_szFilename.c_str() );
                else
                {
                    m_pos += sstRead;
                    THROWBADJSONSTREAMERRNO( errno, "JsonLinuxInputStream::ReadChar(): missing multicode bytes before EOF [%ld].", 
                        m_szFilename.c_str(), nCharsLeft * sizeof(_tyChar) - sstRead );
                }
            }
            m_pos += sstRead;
    ...
    bool FReadChar( _tyUtfChar & _rtch, bool _fThrowOnEOF, const char * _pcEOFMessage )
    {
        assert( FOpened() );
        if ( m_fHasLookahead )
        {
            _rtch = m_utcLookahead;
            m_fHasLookahead = false;
            return true;
        }
        ...
        if ( sstRead != sizeof tchFirst )
        {
            ...
            {
                m_pos += sstRead;
                THROWBADJSONSTREAMERRNO( errno, "JsonLinuxInputStream::FReadChar(): read() for file [%s] had [%ld] leftover bytes.", m_szFilename.c_str(), sstRead );
            }
            else
            {
                if ( _fThrowOnEOF )
        ...
        if ( nCharsLeft )
        {
            ...
            {
                if ( -1 == sstRead )
                    THROWBADJSONSTREAMERRNO( errno, "JsonLinuxInputStream::FReadChar(): read() failed for file [%s] reading multicode UTF.", m_szFilename.c_str() );
                else
                {
                    m_pos += sstRead;
                    THROWBADJSONSTREAMERRNO( errno, "JsonLinuxInputStream::FReadChar(): missing multicode bytes before EOF [%ld].", 
                        m_szFilename.c_str(), nCharsLeft * sizeof(_tyChar) - sstRead );
                }
            }
            m_pos += sstRead;
    ...
    void PushBackLastChar( bool _fMightBeWhitespace = false )
    {
        ...
        assert( _fMightBeWhitespace || !_tyCharTraits::FIsWhitespace( m_utcLookahead ) );
        if ( !_tyCharTraits::FIsWhitespace( m_utcLookahead ) )
            m_fHasLookahead = true;
    }
...
protected:
    ...
    _tyUtfChar m_utcLookahead; // Everytime we read a UTF character we put it in the m_utcLookahead and clear that we have a lookahead.
...
public:
    ...
    void SkipWhitespace() 
    {
        ...
        for ( ; ; )
        {
            if ( FEOF() )
                return; // We have skipped the whitespace until we hit EOF.
            unsigned int nCharsLeft = m_utcLookahead.NSetFirstChar( *m_ptcMappedCur++ );
            if ( ( m_ptcMappedEnd - m_ptcMappedCur ) < nCharsLeft )
                THROWBADJSONSTREAMERRNO( errno, "JsonLinuxInputMemMappedStream::SkipWhitespace(): missing multicode bytes before EOF [%ld].", 
                    m_szFilename.c_str(), ( nCharsLeft - ( m_ptcMappedEnd - m_ptcMappedCur ) ) * sizeof(_tyChar) );     
            memcpy( m_utcLookahead.begin()+1, m_ptcMappedCur, nCharsLeft * sizeof(_tyChar) );
            m_ptcMappedCur += nCharsLeft;
            if ( !_tyCharTraits::FIsWhitespace( m_utcLookahead ) )
            {
                m_fHasLookahead = true;
                return;
    ...
    {
        assert( FOpened() );
        return ( ( m_ptcMappedCur - (_tyChar*)m_pvMapped ) - ( m_fHasLookahead ? m_utcLookahead.NCodes() : 0 ) ) * sizeof(_tyChar);
    }
    // Read a single character from the file - always throw on EOF.
    _tyUtfChar ReadChar( const char * _pcEOFMessage )
    {
        ...
        unsigned int nCharsLeft = m_utcLookahead.NSetFirstChar( *m_ptcMappedCur++ );
        if ( ( m_ptcMappedEnd - m_ptcMappedCur ) < nCharsLeft )
            THROWBADJSONSTREAMERRNO( errno, "JsonLinuxInputMemMappedStream::ReadChar(): missing multicode bytes before EOF [%ld].", 
                m_szFilename.c_str(), ( nCharsLeft - ( m_ptcMappedEnd - m_ptcMappedCur ) ) * sizeof(_tyChar) );     
        memcpy( m_utcLookahead.begin()+1, m_ptcMappedCur, nCharsLeft * sizeof(_tyChar) );
        m_ptcMappedCur += nCharsLeft;
        return m_utcLookahead;
    ...
    bool FReadChar( _tyUtfChar & _rtch, bool _fThrowOnEOF, const char * _pcEOFMessage )
    {
        ...
        unsigned int nCharsLeft = m_utcLookahead.NSetFirstChar( *m_ptcMappedCur++ );
        if ( ( m_ptcMappedEnd - m_ptcMappedCur ) < nCharsLeft )
            THROWBADJSONSTREAMERRNO( errno, "JsonLinuxInputMemMappedStream::FReadChar(): missing multicode bytes before EOF [%ld].", 
                m_szFilename.c_str(), ( nCharsLeft - ( m_ptcMappedEnd - m_ptcMappedCur ) ) * sizeof(_tyChar) );
        memcpy( m_utcLookahead.begin()+1, m_ptcMappedCur, nCharsLeft * sizeof(_tyChar) );
        m_ptcMappedCur += nCharsLeft;
        _rtch = m_utcLookahead;
...
protected:
    ...
    size_t m_stMapped{};
    int m_fd{-1}; // file descriptor.
    _tyUtfChar m_utcLookahead{0}; // Everytime we read a character we put it in the m_utcLookahead and clear that we have a lookahead.
    bool m_fHasLookahead{false};
};
...
public:
    ...
    JsonReadCursor( bool _fMaintainJSONPath = false )
    {
        if ( _fMaintainJSONPath )
            m_strPath = _tyCharTraits::s_tcDollarSign; // We are at the root.
    }
    ...
    void _ReadNumber( _tyChar _tcFirst, bool _fAtRootElement, _tyStdStr & _rstrRead ) const
    {
        ...
        *ptcCur++ = _tcFirst;
        ...
        _tyUtfChar utcCurrentChar; // maintained as the current character.
        if ( _tyCharTraits::s_tcMinus == _tcFirst )
        {
            utcCurrentChar = m_pis->ReadChar( "JsonReadCursor::_ReadNumber(): Hit EOF looking for a digit after a minus." ); // This may not be EOF.
            *ptcCur++ = utcCurrentChar.GetChar();
            if ( ( utcCurrentChar.GetChar() < _tyCharTraits::s_tc0 ) || ( utcCurrentChar.GetChar() > _tyCharTraits::s_tc9 ) )
                THROWBADJSONSTREAM( "JsonReadCursor::_ReadNumber(): Found [%TC] when looking for digit after a minus.", utcCurrentChar.GetChar() );
            fZeroFirst = ( _tyCharTraits::s_tc0 == utcCurrentChar.GetChar() );
        }
        // If we are the root element then we may encounter EOF and have that not be an error when reading some of the values.
        if ( fZeroFirst )
        {
            bool fFoundChar = m_pis->FReadChar( utcCurrentChar, !_fAtRootElement, "JsonReadCursor::_ReadNumber(): Hit EOF looking for something after a leading zero." ); // Throw on EOF if we aren't at the root element.
            if ( !fFoundChar )
        ...
        else // ( !fZeroFirst )
        {
            ...
            for(;;)
            {            
                bool fFoundChar = m_pis->FReadChar( utcCurrentChar, !_fAtRootElement, "JsonReadCursor::_ReadNumber(): Hit EOF looking for a non-number." ); // Throw on EOF if we aren't at the root element.
                if ( !fFoundChar )
                    goto Label_DreadedLabel; // Then we read until the end of file for a JSON file containing a single number as its only element.
                if ( ( utcCurrentChar.GetChar() >= _tyCharTraits::s_tc0 ) && ( utcCurrentChar.GetChar() <= _tyCharTraits::s_tc9 ) )
                    lambdaAddCharNum( utcCurrentChar.GetChar() );
                else
                    break;
            } 
        }
    
        if ( utcCurrentChar.GetChar() == _tyCharTraits::s_tcPeriod )
        {
            lambdaAddCharNum( utcCurrentChar.GetChar() ); // Don't miss your period.
            // Then according to the JSON spec we must have at least one digit here.
            utcCurrentChar = m_pis->ReadChar( "JsonReadCursor::_ReadNumber(): Hit EOF looking for a digit after a decimal point." ); // throw on EOF.
            if ( ( utcCurrentChar.GetChar() < _tyCharTraits::s_tc0 ) || ( utcCurrentChar.GetChar() > _tyCharTraits::s_tc9 ) )
                THROWBADJSONSTREAM( "JsonReadCursor::_ReadNumber(): Found [%TC] when looking for digit after a decimal point.", utcCurrentChar.GetChar() );
            // Now we expect a digit, 'e', 'E', EOF or something else that our parent can check out.
            lambdaAddCharNum( utcCurrentChar.GetChar() );
            for(;;)
            {            
                bool fFoundChar = m_pis->FReadChar( utcCurrentChar, !_fAtRootElement, "JsonReadCursor::_ReadNumber(): Hit EOF looking for a non-number after the period." ); // Throw on EOF if we aren't at the root element.
                if ( !fFoundChar )
                    goto Label_DreadedLabel; // Then we read until the end of file for a JSON file containing a single number as its only element.            
                if ( ( utcCurrentChar.GetChar() >= _tyCharTraits::s_tc0 ) && ( utcCurrentChar.GetChar() <= _tyCharTraits::s_tc9 ) )
                    lambdaAddCharNum( utcCurrentChar.GetChar() );
        ...
        if (    ( utcCurrentChar.GetChar() == _tyCharTraits::s_tcE ) ||
                ( utcCurrentChar.GetChar() == _tyCharTraits::s_tce ) )
        {
            lambdaAddCharNum( utcCurrentChar.GetChar() );
            // Then we might see a plus or a minus or a number - but we cannot see EOF here correctly:
            utcCurrentChar = m_pis->ReadChar( "JsonReadCursor::_ReadNumber(): Hit EOF looking for a digit after an exponent indicator." ); // Throws on EOF.
            lambdaAddCharNum( utcCurrentChar.GetChar() );
            if (    ( utcCurrentChar.GetChar() == _tyCharTraits::s_tcMinus ) ||
                   ( utcCurrentChar.GetChar() == _tyCharTraits::s_tcPlus ) )
            {
                utcCurrentChar = m_pis->ReadChar( "JsonReadCursor::_ReadNumber(): Hit EOF looking for a digit after an exponent indicator with plus/minus." ); // Then we must still find a number after - throws on EOF.
                lambdaAddCharNum( utcCurrentChar.GetChar() );
            }
            // Now we must see at least one digit so for the first read we throw.
            if ( ( utcCurrentChar.GetChar() < _tyCharTraits::s_tc0 ) || ( utcCurrentChar.GetChar() > _tyCharTraits::s_tc9 ) )
                THROWBADJSONSTREAM( "JsonReadCursor::_ReadNumber(): Found [%TC] when looking for digit after a exponent indicator (e/E).", utcCurrentChar.GetChar() );
            // We have satisfied "at least a digit after an exponent indicator" - now we might just hit EOF or anything else after this...
            for(;;)
            {
                bool fFoundChar = m_pis->FReadChar( utcCurrentChar, !_fAtRootElement, "JsonReadCursor::_ReadNumber(): Hit EOF looking for a non-number after the exponent indicator." ); // Throw on EOF if we aren't at the root element.
                if ( !fFoundChar )
                    goto Label_DreadedLabel; // Then we read until the end of file for a JSON file containing a single number as its only element.            
                if ( ( utcCurrentChar.GetChar() >= _tyCharTraits::s_tc0 ) && ( utcCurrentChar.GetChar() <= _tyCharTraits::s_tc9 ) )
                    lambdaAddCharNum( utcCurrentChar.GetChar() );
...
Label_DreadedLabel: // Just way too easy to do it this way.
    ...
    void _SkipNumber( _tyChar _tcFirst, bool _fAtRootElement ) const
    {
        ...
        if ( _tyCharTraits::s_tcMinus == _tcFirst )
        {
            utcCurrentChar = m_pis->ReadChar( "JsonReadCursor::_SkipNumber(): Hit EOF looking for a digit after a minus." ); // This may not be EOF.
            if ( ( utcCurrentChar.GetChar() < _tyCharTraits::s_tc0 ) || ( utcCurrentChar.GetChar() > _tyCharTraits::s_tc9 ) )
                THROWBADJSONSTREAM( "JsonReadCursor::_SkipNumber(): Found [%TC] when looking for digit after a minus.", utcCurrentChar.GetChar() );
            fZeroFirst = ( _tyCharTraits::s_tc0 == utcCurrentChar.GetChar() );
        }
        // If we are the root element then we may encounter EOF and have that not be an error when reading some of the values.
        if ( fZeroFirst )
        {
            bool fFoundChar = m_pis->FReadChar( utcCurrentChar, !_fAtRootElement, "JsonReadCursor::_SkipNumber(): Hit EOF looking for something after a leading zero." ); // Throw on EOF if we aren't at the root element.
            if ( !fFoundChar )
        ...
        else // ( !fZeroFirst )
        {
            ...
            do
            {            
                bool fFoundChar = m_pis->FReadChar( utcCurrentChar, !_fAtRootElement, "JsonReadCursor::_SkipNumber(): Hit EOF looking for a non-number." ); // Throw on EOF if we aren't at the root element.
                if ( !fFoundChar )
                    return; // Then we read until the end of file for a JSON file containing a single number as its only element.
            } 
            while ( ( utcCurrentChar.GetChar() >= _tyCharTraits::s_tc0 ) && ( utcCurrentChar.GetChar() <= _tyCharTraits::s_tc9 ) );
        }
    
        if ( utcCurrentChar.GetChar() == _tyCharTraits::s_tcPeriod )
        {
            // Then according to the JSON spec we must have at least one digit here.
            utcCurrentChar = m_pis->ReadChar( "JsonReadCursor::_SkipNumber(): Hit EOF looking for a digit after a decimal point." ); // throw on EOF.
            if ( ( utcCurrentChar.GetChar() < _tyCharTraits::s_tc0 ) || ( utcCurrentChar.GetChar() > _tyCharTraits::s_tc9 ) )
                THROWBADJSONSTREAM( "JsonReadCursor::_SkipNumber(): Found [%TC] when looking for digit after a decimal point.", utcCurrentChar.GetChar() );
            // Now we expect a digit, 'e', 'E', EOF or something else that our parent can check out.
            do
            {            
                bool fFoundChar = m_pis->FReadChar( utcCurrentChar, !_fAtRootElement, "JsonReadCursor::_SkipNumber(): Hit EOF looking for a non-number after the period." ); // Throw on EOF if we aren't at the root element.
                if ( !fFoundChar )
                    return; // Then we read until the end of file for a JSON file containing a single number as its only element.            
            }
            while ( ( utcCurrentChar.GetChar() >= _tyCharTraits::s_tc0 ) && ( utcCurrentChar.GetChar() <= _tyCharTraits::s_tc9 ) );
        ...
        {
            // Then we might see a plus or a minus or a number - but we cannot see EOF here correctly:
            utcCurrentChar = m_pis->ReadChar( "JsonReadCursor::_SkipNumber(): Hit EOF looking for a digit after an exponent indicator." ); // Throws on EOF.
            if (    ( utcCurrentChar.GetChar() == _tyCharTraits::s_tcMinus ) ||
                    ( utcCurrentChar.GetChar() == _tyCharTraits::s_tcPlus ) )
                utcCurrentChar = m_pis->ReadChar( "JsonReadCursor::_SkipNumber(): Hit EOF looking for a digit after an exponent indicator with plus/minus." ); // Then we must still find a number after - throws on EOF.
            // Now we must see at least one digit so for the first read we throw.
            if ( ( utcCurrentChar.GetChar() < _tyCharTraits::s_tc0 ) || ( utcCurrentChar.GetChar() > _tyCharTraits::s_tc9 ) )
                THROWBADJSONSTREAM( "JsonReadCursor::_SkipNumber(): Found [%TC] when looking for digit after an exponent indicator (e/E).", utcCurrentChar.GetChar() );
            // We have satisfied "at least a digit after an exponent indicator" - now we might just hit EOF or anything else after this...
            do
            {
                bool fFoundChar = m_pis->FReadChar( utcCurrentChar, !_fAtRootElement, "JsonReadCursor::_ReadNumber(): Hit EOF looking for a non-number after the exponent indicator." ); // Throw on EOF if we aren't at the root element.
    ...
    void _ReadString( _tyStdStr & _rstrRead ) const
    {
        ...
        for( ; ; )
        {
            _tyUtfChar utcCurrentChar = m_pis->ReadChar( "JsonReadCursor::_ReadString(): EOF found looking for end of string." ); // throws on EOF.
            if ( utcCurrentChar.FSingleCode() )
            {
                if ( _tyCharTraits::s_tcDoubleQuote == utcCurrentChar.GetChar() )
                    break; // We have reached EOS.
                if ( _tyCharTraits::s_tcBackSlash == utcCurrentChar.GetChar() )
                {
                    utcCurrentChar = m_pis->ReadChar( "JsonReadCursor::_ReadString(): EOF finding completion of backslash escape for string." ); // throws on EOF.
                    if ( !utcCurrentChar.FSingleCode() )
                        THROWBADJSONSTREAM( "JsonReadCursor::_ReadString(): Found multicode start char after a backslash escape char." );
                    switch( utcCurrentChar.GetChar() )
                    {
                        case _tyCharTraits::s_tcDoubleQuote:
                        case _tyCharTraits::s_tcBackSlash:
                        case _tyCharTraits::s_tcForwardSlash:
                            break;   // Just use utcCurrentChar.
                        case _tyCharTraits::s_tcb:
                            utcCurrentChar.SetSingleCode( _tyCharTraits::s_tcBackSpace );
                            break;
                        case _tyCharTraits::s_tcf:
                            utcCurrentChar.SetSingleCode( _tyCharTraits::s_tcFormFeed );
                            break;
                        case _tyCharTraits::s_tcn:
                            utcCurrentChar.SetSingleCode( _tyCharTraits::s_tcNewline );
                            break;
                        case _tyCharTraits::s_tcr:
                            utcCurrentChar.SetSingleCode( _tyCharTraits::s_tcCarriageReturn );
                            break;
                        case _tyCharTraits::s_tct:
                            utcCurrentChar.SetSingleCode( _tyCharTraits::s_tcTab );
                            break;
                        case _tyCharTraits::s_tcu:
                        {
                            // Note that we are not doing *anything* with this - i.e. we are reading it as is. It could specify a bogus UTF prefix/suffix and we wouldn't know it.
                            // When written out this will not be written escaped - i.e. if we copy this using the standard copying templates below. It will then be written out as
                            //  if it were UTF and then we might have an issue. Essentially there is no error checking on the formation of a valid UTF sequence here so you could form anything
                            //  at all you wanted to...
                            ...
                            {
                                utcCurrentChar = m_pis->ReadChar( "JsonReadCursor::_ReadString(): EOF found looking for 4 hex digits following \\u." ); // throws on EOF.
                                if ( !utcCurrentChar.FSingleCode() )
                                    THROWBADJSONSTREAM( "JsonReadCursor::_ReadString(): Found multicode start char instead of a hex digit following a \\u." );
                                if ( ( utcCurrentChar.GetChar() >= _tyCharTraits::s_tc0 ) && ( utcCurrentChar.GetChar() <= _tyCharTraits::s_tc9 ) )
                                    uHex += uCurrentMultiplier * ( utcCurrentChar.GetChar - _tyCharTraits::s_tc0 );
                                else
                                if ( ( utcCurrentChar.GetChar() >= _tyCharTraits::s_tca ) && ( utcCurrentChar.GetChar() <= _tyCharTraits::s_tcf ) )
                                    uHex += uCurrentMultiplier * ( 10 + ( utcCurrentChar.GetChar() - _tyCharTraits::s_tca ) );
                                else
                                if ( ( utcCurrentChar.GetChar() >= _tyCharTraits::s_tcA ) && ( utcCurrentChar.GetChar() <= _tyCharTraits::s_tcF ) )
                                    uHex += uCurrentMultiplier * ( 10 + ( utcCurrentChar.GetChar() - _tyCharTraits::s_tcA ) );
                                else
                                    THROWBADJSONSTREAM( "JsonReadCursor::_ReadString(): Found [%TC] when looking for digit following \\u.", utcCurrentChar.GetChar() );
                            }
                            ...
                            utcCurrentChar.SetSingleCode( (_tyChar)uHex, true ); // "true" means we will force treating this single character even though it may be a lead or suffix UTF code.
                            break;
                        }
                        default:
                            THROWBADJSONSTREAM( "JsonReadCursor::_ReadString(): Found [%TC] when looking for competetion of backslash when reading string.", utcCurrentChar.GetChar() );
                            break;
                    }
                }
            }
            if ( ( ptchCur + utcCurrentChar.NCodes() ) > ( rgtcBuffer + knLenBuffer ) )
            {
                assert( ptchCur <= rgtcBuffer + knLenBuffer );
                *ptchCur = 0;
                _rstrRead += rgtcBuffer;
                ptchCur = rgtcBuffer;
            }
            memcpy( ptchCur, utcCurrentChar.begin(), utcCurrentChar.NCodes() * sizeof(_tyChar) );
            ptchCur += utcCurrentChar.NCodes();
        }
    ...
    void _SkipRemainingString() const
    {
        ...
        for( ; ; )
        {
            _tyUtfChar utcCurrentChar = m_pis->ReadChar( "JsonReadCursor::_SkipRemainingString(): EOF found looking for end of string." ); // throws on EOF.
            if ( ( utcCurrentChar.FSingleCode() )
            {
                if ( _tyCharTraits::s_tcDoubleQuote == utcCurrentChar.GetChar() )
                    break; // We have reached EOS.
                if ( _tyCharTraits::s_tcBackSlash == utcCurrentChar.GetChar() )
                {
                    utcCurrentChar = m_pis->ReadChar( "JsonReadCursor::_SkipRemainingString(): EOF finding completion of backslash escape for string." ); // throws on EOF.
                    if ( !utcCurrentChar.FSingleCode() )
                        THROWBADJSONSTREAM( "JsonReadCursor::_SkipRemainingString(): Found multicode start char after a backslash escape char." );
                    switch( utcCurrentChar.GetChar() )
                    {
                        case _tyCharTraits::s_tcDoubleQuote:
                        case _tyCharTraits::s_tcBackSlash:
                        ...
                        case _tyCharTraits::s_tcu:
                        {
                            ...
                            for ( int n=0; n < 4; ++n )
                            {
                                utcCurrentChar = m_pis->ReadChar( "JsonReadCursor::_SkipRemainingString(): EOF found looking for 4 hex digits following \\u." ); // throws on EOF.
                                if ( !utcCurrentChar.FSingleCode() )
                                    THROWBADJSONSTREAM( "JsonReadCursor::_SkipRemainingString(): Found multicode start char instead of a hex digit following a \\u." );
                                if ( !( ( ( utcCurrentChar.GetChar() >= _tyCharTraits::s_tc0 ) && ( utcCurrentChar.GetChar() <= _tyCharTraits::s_tc9 ) ) ||
                                        ( ( utcCurrentChar.GetChar() >= _tyCharTraits::s_tca ) && ( utcCurrentChar.GetChar() <= _tyCharTraits::s_tcf ) ) ||
                                        ( ( utcCurrentChar.GetChar() >= _tyCharTraits::s_tcA ) && ( utcCurrentChar.GetChar() <= _tyCharTraits::s_tcF ) ) ) ) 
                                    THROWBADJSONSTREAM( "JsonReadCursor::_SkipRemainingString(): Found [%TC] when looking for digit following \\u.", utcCurrentChar.GetChar() );
                            }
                        }
                        default:
                            THROWBADJSONSTREAM( "JsonReadCursor::_SkipRemainingString(): Found [%TC] when looking for competetion of backslash when reading string.", utcCurrentChar.GetChar() );
                            break;
                    }
                }
    ...
    void _SkipFixed( const char * _pcSkip ) const
    {
        ...
        _tyUtfChar utcCurrentChar;
        for ( ; !!*pcCur && ( _tyChar( *pcCur ) == (  ) ); ++pcCur )
        {
            utcCurrentChar = m_pis->ReadChar("JsonReadCursor::_SkipFixed(): Hit EOF trying to skip fixed string.");
            if ( !utcCurrentChar.FSingleCode() )
                THROWBADJSONSTREAM( "JsonReadCursor::_SkipFixed(): Found multicode start char instead of [%c] when trying to skip [%s].", *pcCur, _pcSkip );
            if ( *pcCur != utcCurrentChar.GetChar() )
                break;
        }
        if ( !!*pcCur )
            THROWBADJSONSTREAM( "JsonReadCursor::_SkipFixed(): Trying to skip[%s], instead of [%c] found [%TC].", _pcSkip, *pcCur, utcCurrentChar.GetChar() );
    }
    ...
    void _SkipValue()
    {
        ...
        _tyUtfChar utcCurrentChar = m_pis->ReadChar( "JsonReadCursor::_SkipValue(): EOF on first value char." ); // throws on EOF.
        if ( !utcCurrentChar.FSingleCode() )
            THROWBADJSONSTREAM( "JsonReadCursor::_SkipValue(): Multicode character when searching for value start character." );
        EJsonValueType jvtCur = _tyJsonValue::GetJvtTypeFromChar( utcCurrentChar.GetChar() );
        if ( ejvtJsonValueTypeCount == jvtCur )
            THROWBADJSONSTREAM( "JsonReadCursor::_SkipValue(): Found [%TC] when looking for value starting character.", utcCurrentChar.GetChar() );
        ...
        if ( ejvtArray ==  jvtCur )
            ...
            _SkipSimpleValue( jvtCur, utcCurrentChar.GetChar(), false ); // we assume we aren't at the root element here since we shouldn't get here - we would have a context.
    }
    ...
    {
        m_pis->SkipWhitespace();
        _tyUtfChar utcCurrentChar = m_pis->ReadChar( "JsonReadCursor::_SkipWholeObject(): EOF after begin bracket." ); // throws on EOF.
        if ( !utcCurrentChar.FSingleCode() )
            THROWBADJSONSTREAM( "JsonReadCursor::_SkipWholeObject(): Multicode character when searching for double quote or object end." );
        while ( _tyCharTraits::s_tcDoubleQuote == utcCurrentChar.GetChar() )
        {
            _SkipRemainingString(); // Skip the key.
            m_pis->SkipWhitespace();
            utcCurrentChar = m_pis->ReadChar( "JsonReadCursor::_SkipWholeObject(): EOF looking for colon." ); // throws on EOF.
            if ( !utcCurrentChar.FSingleCode() )
                THROWBADJSONSTREAM( "JsonReadCursor::_SkipWholeObject(): Multicode character when searching for colon." );
            if ( _tyCharTraits::s_tcColon != utcCurrentChar.GetChar() )
                THROWBADJSONSTREAM( "JsonReadCursor::_SkipWholeObject(): Found [%TC] when looking for colon.", utcCurrentChar.GetChar() );
            _SkipValue(); // Skip the value.
            m_pis->SkipWhitespace();
            utcCurrentChar = m_pis->ReadChar( "JsonReadCursor::_SkipWholeObject(): EOF looking comma or end of object." ); // throws on EOF.
            if ( !utcCurrentChar.FSingleCode() )
                THROWBADJSONSTREAM( "JsonReadCursor::_SkipWholeObject(): Multicode character when searching for comma." );
            if ( _tyCharTraits::s_tcComma == utcCurrentChar.GetChar() )
            {
                m_pis->SkipWhitespace();
                utcCurrentChar = m_pis->ReadChar( "JsonReadCursor::_SkipWholeObject(): EOF looking double quote." ); // throws on EOF.
                if ( !utcCurrentChar.FSingleCode() )
                    THROWBADJSONSTREAM( "JsonReadCursor::_SkipWholeObject(): Multicode character when searching for double quote." );
                if ( _tyCharTraits::s_tcDoubleQuote != utcCurrentChar.GetChar() )
                    THROWBADJSONSTREAM( "JsonReadCursor::_SkipWholeObject(): Found [%TC] when looking for double quote.", utcCurrentChar.GetChar() );
            }
            else
                break;
        }
        if ( _tyCharTraits::s_tcRightCurlyBr != utcCurrentChar.GetChar() )
            THROWBADJSONSTREAM( "JsonReadCursor::_SkipWholeObject(): Found [%TC] when looking for double quote or comma or right bracket.", utcCurrentChar.GetChar() );
    }
    // We will have read the first '[' of the array.
    void _SkipWholeArray()
    {
        m_pis->SkipWhitespace();
        _tyUtfChar utcCurrentChar = m_pis->ReadChar( "JsonReadCursor::_SkipWholeArray(): EOF after begin bracket." ); // throws on EOF.
        if ( !utcCurrentChar.FSingleCode() )
            THROWBADJSONSTREAM( "JsonReadCursor::_SkipWholeArray(): Multicode character when searching for first value char or end bracket." );
        if ( _tyCharTraits::s_tcRightSquareBr != utcCurrentChar.GetChar() )
            m_pis->PushBackLastChar(); // Don't send an expected character as it is from the set of value-starting characters.
        while ( _tyCharTraits::s_tcRightSquareBr != utcCurrentChar.GetChar() )
        {
            _SkipValue(); // _SkipValue() calls SkipWhitespace() first.
            m_pis->SkipWhitespace();
            utcCurrentChar = m_pis->ReadChar( "JsonReadCursor::_SkipWholeArray(): EOF looking for comma." ); // throws on EOF.
            if ( !utcCurrentChar.FSingleCode() )
                THROWBADJSONSTREAM( "JsonReadCursor::_SkipWholeArray(): Multicode character when looking for comma." );
            if ( _tyCharTraits::s_tcComma == utcCurrentChar.GetChar() )
                continue;
            if ( _tyCharTraits::s_tcRightSquareBr != utcCurrentChar.GetChar() )
                THROWBADJSONSTREAM( "JsonReadCursor::_SkipWholeArray(): Found [%TC] when looking for comma or array end.", utcCurrentChar.GetChar() );
        }
    }
    ...
    void _SkipObject( _tyJsonReadContext & _rjrx )
    {
        ...
        m_pis->SkipWhitespace();
        _tyUtfChar utcCurrentChar = m_pis->ReadChar( "JsonReadCursor::_SkipObject(): EOF looking for end object } or comma." ); // throws on EOF.
        if ( !utcCurrentChar.FSingleCode() )
            THROWBADJSONSTREAM( "JsonReadCursor::_SkipObject(): Multicode character when looking for end object } or comma." );
        while ( _tyCharTraits::s_tcRightCurlyBr != utcCurrentChar.GetChar() )
        {
            if ( _tyCharTraits::s_tcComma != utcCurrentChar.GetChar() )
                THROWBADJSONSTREAM( "JsonReadCursor::_SkipObject(): Found [%TC] when looking for comma or object end.", utcCurrentChar.GetChar() );
            m_pis->SkipWhitespace();
            utcCurrentChar = m_pis->ReadChar( "JsonReadCursor::_SkipObject(): EOF looking for double quote." ); // throws on EOF.
            if ( !utcCurrentChar.FSingleCode() )
                THROWBADJSONSTREAM( "JsonReadCursor::_SkipObject(): Multicode character when looking for double quote." );
            if ( _tyCharTraits::s_tcDoubleQuote != utcCurrentChar.GetChar() )
                THROWBADJSONSTREAM( "JsonReadCursor::_SkipObject(): Found [%TC] when looking key start double quote.", utcCurrentChar.GetChar() );
            _SkipRemainingString();
            m_pis->SkipWhitespace();
            utcCurrentChar = m_pis->ReadChar( "JsonReadCursor::_SkipObject(): EOF looking for colon." ); // throws on EOF.
            if ( !utcCurrentChar.FSingleCode() )
                THROWBADJSONSTREAM( "JsonReadCursor::_SkipObject(): Multicode character when looking for colon." );
            if ( _tyCharTraits::s_tcColon != utcCurrentChar.GetChar() )
                THROWBADJSONSTREAM( "JsonReadCursor::_SkipObject(): Found [%TC] when looking for colon.", utcCurrentChar.GetChar() );
            _SkipValue();
            m_pis->SkipWhitespace();
            utcCurrentChar = m_pis->ReadChar( "JsonReadCursor::_SkipObject(): EOF looking for end object } or comma." ); // throws on EOF.
            if ( !utcCurrentChar.FSingleCode() )
                THROWBADJSONSTREAM( "JsonReadCursor::_SkipObject(): Multicode character when looking for end object } or comma." );
        }
    ...
    void _SkipArray( _tyJsonReadContext & _rjrx )
    {
        ...
        m_pis->SkipWhitespace();
        _tyUtfChar utcCurrentChar = m_pis->ReadChar( "JsonReadCursor::_SkipArray(): EOF looking for end array ] or comma." ); // throws on EOF.
        if ( !utcCurrentChar.FSingleCode() )
            THROWBADJSONSTREAM( "JsonReadCursor::_SkipArray(): Multicode character when looking for end array ] or comma." );
        while ( _tyCharTraits::s_tcRightSquareBr != utcCurrentChar.GetChar() )
        {
            if ( _tyCharTraits::s_tcComma != utcCurrentChar.GetChar() )
                THROWBADJSONSTREAM( "JsonReadCursor::_SkipArray(): Found [%TC] when looking for comma or array end.", utcCurrentChar.GetChar() );
            m_pis->SkipWhitespace();
            _SkipValue();
            m_pis->SkipWhitespace();
            utcCurrentChar = m_pis->ReadChar( "JsonReadCursor::_SkipArray(): EOF looking for end array ] or comma." ); // throws on EOF.
            if ( !utcCurrentChar.FSingleCode() )
                THROWBADJSONSTREAM( "JsonReadCursor::_SkipArray(): Multicode character when looking for end array ] or comma." );
        }
    ...
    bool FNextElement()
    {
        ...
        m_pis->SkipWhitespace();
        _tyUtfChar utcCurrentChar = m_pis->ReadChar( "JsonReadCursor::FNextElement(): EOF looking for end object/array }/] or comma." ); // throws on EOF.
        if ( !utcCurrentChar.FSingleCode() )
            THROWBADJSONSTREAM( "JsonReadCursor::FNextElement(): Multicode character when looking for end object/array }/] or comma." );
        if ( ejvtObject == m_pjrxCurrent->m_pjrxNext->JvtGetValueType() )
        {
            ...
            if ( _tyCharTraits::s_tcComma != utcCurrentChar.GetChar() )
                THROWBADJSONSTREAM( "JsonReadCursor::FNextElement(): Found [%TC] when looking for comma or object end.", utcCurrentChar.GetChar() );
            // Now we will read the key string of the next element into <pjoCur>.
            m_pis->SkipWhitespace();
            utcCurrentChar = m_pis->ReadChar( "JsonReadCursor::FNextElement(): EOF looking for double quote." ); // throws on EOF.
            if ( !utcCurrentChar.FSingleCode() )
                THROWBADJSONSTREAM( "JsonReadCursor::FNextElement(): Multicode character when looking for double quote." );
            if ( _tyCharTraits::s_tcDoubleQuote != utcCurrentChar.GetChar() )
                THROWBADJSONSTREAM( "JsonReadCursor::FNextElement(): Found [%TC] when looking key start double quote.", utcCurrentChar.GetChar() );
            ...
            m_pis->SkipWhitespace();
            utcCurrentChar = m_pis->ReadChar( "JsonReadCursor::FNextElement(): EOF looking for colon." ); // throws on EOF.
            if ( !utcCurrentChar.FSingleCode() )
                THROWBADJSONSTREAM( "JsonReadCursor::FNextElement(): Multicode character when looking for colon." );
            if ( _tyCharTraits::s_tcColon != utcCurrentChar.GetChar() )
                THROWBADJSONSTREAM( "JsonReadCursor::FNextElement(): Found [%TC] when looking for colon.", utcCurrentChar.GetChar() );
        }
        else
        {
            ...
            if ( _tyCharTraits::s_tcComma != utcCurrentChar.GetChar() )
                THROWBADJSONSTREAM( "JsonReadCursor::FNextElement(): Found [%TC] when looking for comma or array end.", utcCurrentChar.GetChar() );
        }
        m_pis->SkipWhitespace();
        ...
        utcCurrentChar = m_pis->ReadChar( "JsonReadCursor::FNextElement(): EOF looking for next object/array value." );
        if ( !utcCurrentChar.FSingleCode() )
            THROWBADJSONSTREAM( "JsonReadCursor::FNextElement(): Multicode character when looking for value start." );
        m_pjrxCurrent->m_tcFirst = utcCurrentChar.GetChar();
    ...
    void AttachRoot( t_tyJsonInputStream & _ris )
    {
        ...
        // The first non-whitespace character tells us what the value type is:
        _tyUtfChar utcCurrentChar = _ris.ReadChar( "JsonReadCursor::AttachRoot(): Empty JSON file." );
        if ( !utcCurrentChar.FSingleCode() )
            THROWBADJSONSTREAM( "JsonReadCursor::AttachRoot(): JSON file starts with multicode UTF character." );
        pjrxRoot->m_tcFirst = utcCurrentChar.GetChar() ;
    ...
    bool FMoveDown()
    {
        ...
        if ( ejvtObject == m_pjrxCurrent->JvtGetValueType() )
        {
            ...
            _tyUtfChar utcCurrentChar = m_pis->ReadChar( "JsonReadCursor::FMoveDown(): EOF looking for first character of an object." ); // throws on eof.
            if ( !utcCurrentChar.FSingleCode() )
                THROWBADJSONSTREAM( "JsonReadCursor::FMoveDown(): Multicode UTF character looking for first character of an object." );
            if ( ( _tyCharTraits::s_tcRightCurlyBr != utcCurrentChar.GetChar() ) && ( _tyCharTraits::s_tcDoubleQuote != utcCurrentChar.GetChar() ) )
                THROWBADJSONSTREAM( "JsonReadCursor::FMoveDown(): Found [%TC] when looking for first character of object.", utcCurrentChar.GetChar() );
            ...
            if ( _tyCharTraits::s_tcDoubleQuote == utcCurrentChar.GetChar() )
            {
                ...
                m_pis->SkipWhitespace();
                utcCurrentChar = m_pis->ReadChar( "JsonReadCursor::FMoveDown(): EOF looking for colon on first object pair." ); // throws on eof.
                if ( !utcCurrentChar.FSingleCode() )
                    THROWBADJSONSTREAM( "JsonReadCursor::FMoveDown(): Multicode UTF character looking for colon on first object pair." );
                if ( _tyCharTraits::s_tcColon != utcCurrentChar.GetChar() )
                    THROWBADJSONSTREAM( "JsonReadCursor::FMoveDown(): Found [%TC] when looking for colon on first object pair.", utcCurrentChar.GetChar() );
                m_pis->SkipWhitespace();
                ...
                // The first non-whitespace character tells us what the value type is:
                utcCurrentChar = m_pis->ReadChar( "JsonReadCursor::FMoveDown(): EOF looking for first object value." );
                if ( !utcCurrentChar.FSingleCode() )
                    THROWBADJSONSTREAM( "JsonReadCursor::FMoveDown(): Multicode UTF character looking for first object value." );
                pjrxNewRoot->m_tcFirst = utcCurrentChar.GetChar();
        ...
        else // Array.
        {
            ...
            _tyUtfChar utcCurrentChar = m_pis->ReadChar( "JsonReadCursor::FMoveDown(): EOF looking for first character of an array." ); // throws on eof.
            if ( !utcCurrentChar.FSingleCode() )
                THROWBADJSONSTREAM( "JsonReadCursor::FMoveDown(): Multicode UTF character looking for first character of an array." );
            EJsonValueType jvtCur = _tyJsonValue::GetJvtTypeFromChar( utcCurrentChar.GetChar() );
            if ( ( _tyCharTraits::s_tcRightSquareBr != utcCurrentChar.GetChar() ) && ( ejvtJsonValueTypeCount == jvtCur ) )
                THROWBADJSONSTREAM( "JsonReadCursor::FMoveDown(): Found [%TC] when looking for first char of array value.", utcCurrentChar.GetChar() );
...
protected:
    ...
    _tyStdStr m_strPath; // The current pathway for m_pjrxCurrent. This is only maintained when it isn't empty.
};
